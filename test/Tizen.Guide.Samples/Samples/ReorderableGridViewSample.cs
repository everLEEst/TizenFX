...
namespace Tizen.Guide.Samples
{
    ...
    internal class ReorderableGridViewSample : ContentPage, ISample
    {
        ...
        public ReorderableGridViewSample() : base()
        {
            ...
            layouter.ItemReordered += (o, e) =>
            {
                ...
                // Reorder Animation is not actually occurs reorder of source.
                // So the owner of source must reorder the source data here.
    ...
    /************************** Custom Reordable Grid Layouter ***************************/
    public class ReorderableGridLayouter : GridLayouter
    {
        ...
        private Animation reorderAnimation;
        private int itemShiftDuration = 500;
        private int reorderDuration = 500;
        ...
        public override void Initialize(RecyclerView view)
        {
            ...
            // Preview box for reodering place target
        ...
        public override void RequestLayout(float scrollPosition, bool force = false)
        {
            ...
            foreach (RecyclerViewItem item in VisibleItems)
            {
                ...
                {
                    // We need to skip the reordering item unrealization.
        ...
        /* Touch Gesture event callback on item longpressed.
           Gesture have state information and Local Point position.
           Using this informations to implement reorder in here. */
        internal void ItemOnLongPressed(RecyclerViewItem item, LongPressGestureDetector.DetectedEventArgs args)
        {
            ...
            switch (gesture.State) {
                ...
                /* Longpress Started.
                   Create Preview object to show the place to be reordered
                   and raise the longpressed item here. */
                ...
                /* Longpress Finished.
                   Animate reordered item to target place and do actual reorder of source data. */
                ...
                reorderAnimation?.Stop();
                reorderAnimation = new Animation(reorderDuration);
                ...
                reorderAnimation.Finished += (object o, EventArgs e) =>
                {
                    ...
                    {
                    /* Reordered event call.
                       Layouter cannot reorder on collectionView source data, so actual reorder must be happen in the event callback code. */
                ...
                reorderAnimation.Play();
                AnimateItemShift(targetIndex, onReordering.Index);
        ...
        internal void OnPanGestureDetected(object view, PanGestureDetector.DetectedEventArgs args)
        {
            ...
            // 3. Make an animations of reorder to target index.
            if (targetIndex != -1)
            {
        ...
        private void AnimateItemShift(int from, int to)
        {
            ...
            itemShiftAnimation.Stop();
            itemShiftAnimation.Clear();
            itemShiftAnimation.EndAction = Animation.EndActions.Cancel;
